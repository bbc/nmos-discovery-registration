...
Generation of the Node ID should be performed using the version 1 UUID format (stored persistently) where possible in order to provide access to the system MAC address in the final 12 characters of the UUID. Where a v1 UUID is detected it may be used as a method to translate between the NMOS model and Link Layer Discovery Protocol (LLDP) if implemented.
